...
## Verified config — do NOT reintroduce (audit 2026-08-28)
Lessons from a config roast, confirmed against Cecil 9.0.1 source (`cecil.phar`) and a before/after `build` + `diff -rq _site`. All three below are **silently ignored** by Cecil — removing them changes zero output:
- `optimize.gzip: true` — NOT a Cecil key. `optimize` only handles `html`/`css`/`js`/`images`. GitHub Pages never serves pre-compressed files anyway. Don't add it.
- `feeds:` top-level block (`enabled`/`formats`) — does NOT exist in Cecil (no `feeds` key anywhere in `Cecil\`). Feeds are generated by `output.formats` (rss/atom/json/md) + `output.pagetypeformats` + `output.rss: feed.xml`. Removing it does not stop feeds.
- Top-level `locale: 'en_US'` — ignored. Locale lives in `languages[].locale` (en_US/fr_FR); `og:locale` reads `site.language.locale`, not this key.
- **Fonts exception**: font files live in `static/fonts/` (copied verbatim, served at `/fonts/...`), NOT under `assets/`. So `asset()` would 404 — keep the absolute `/fonts/*.woff2` preload paths in `layouts/_default/page.html.twig`. This is the one deliberate exception to the generic "prefer `asset()`" front-end rule.
- `og:image:alt` (in `layouts/partials/metatags.html.twig`) now uses `page.cover.alt|default(opengraph.title)` — the cover `alt` when present, else the SEO title. More descriptive for social shares than the bare title.
